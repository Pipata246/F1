...
### Быстрая проверка без ожидания cron

Экшен **`syncMyDeposits`** (тот же код сканирования, что в `/api/wallet-cron`, но только для твоего `tg_user_id`) вызывается с клиента после пополнения и при открытии вкладки «Баланс», чтобы TonAPI проверился и при совпадении tx + memo вызвался `wallet_credit_deposit`. Лимит интервала: `DEPOSIT_SYNC_MIN_INTERVAL_SEC` (по умолчанию 10 с).

Таблица **`wallet_operations`** — это журнал **уже обработанных** операций (зачисленный депозит, вывод и т.д.). Пока `wallet_credit_deposit` не выполнился, строк там нет: пустая таблица при живых только `deposit_intents` — ожидаемо.

Если TON ушли с кошелька, а баланс в БД не вырос — проверь `TON_DEPOSIT_ADDRESS` (тот же адрес приёма), комментарий в транзакции = `deposit_memo` пользователя, `TONAPI_KEY`, логи `syncMyDeposits` / `/api/wallet-cron` (строки `deposits:`).
