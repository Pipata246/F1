...
const p = path.join(root, 'roulette-server', 'public', 'roulette-ui.js');

const ins = `  clearWinnerCardHighlight(strip) {
    if (!strip) return;
...
const startMark = '  // Получить стабильный индекс цвета';
const endMark = '  // ==================== ACTIONS ====================';
...
s = s.slice(0, a) + ins + s.slice(b);
fs.writeFileSync(p, s);
console.log('spliced', p, 'removed', b - a, 'inserted', ins.length);
