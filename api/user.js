...
function pvpPickBotName() {
  const BOT_NAMES = [
    "tiMeFell","kArmaLoop","NoOneKnows","midNightAir","jusTAlive",
    "loStInFrame","cryStalMood","whYsoQuiet","brOkenSignal","maybeLaterX",
    "iSawNothing","betWeenUs","siLentRush","gOneMissing","tHisIsFine",
    "nEverAgainX","jusTpassingBy","fEltStrange","hoLdOnSlow","iAmNotSure",
    "raNdOmSoul","coldButKind","hAlfAwake","sTillHereX","dOntDisturb",
    "uNrealMoment","noThingMajor","gHostInsideMe","tHoughtAgain","wHatNowThen",
    "noBodySpecial","iAmDifferentX","truStNoEcho","hiDdenSide","jusTAnotherOne",
    "iFyouKnowYouKnow","aBitLostNow","hoWdidIGetHere","lIttleChaos","mIndOffline",
    "whYmeTho","nOtYourType","soMetimeSoon","hEartBuffer","loWkeyAlive",
    "cAntExplainIt","uNknownPath","gOingNowhere","oUtOfSignal","dAyDreamMode",
    "hOldMyThoughts","nExtQuestion","sLightlyWeird","iFEelNothingX","doNotOpenMe",
    "iMightCare","wEirdlyCalm","nOmoreDrama","aLmOstReal","siLentAgain",
    "iForgetThings","wHatWasThat","mAybeNotToday","sLowReaction","jusTnoWords",
    "nOtSoObvious","iThinkTooMuch","rArelySeen","gOwithFlow","sOmeoneElse",
    "hAlfOfMeGone","iDidNothing","dArkButSoft","nOtThisTime","aLoneInRoom",
    "hEreWeGoAgain","juStLetItBe","sTillThinking","noMoreQuestions","mYownWorld",
    "nOideaWhat","sOftInside","wHyAmIHere","iStayQuiet","juStExisting",
    "rAndomAgain","tOuchOfMadness","iCantDecide","sOmeKindOfMe","gOawayPlease",
    "nOtThatDeep","iSaidNothing","hArdToExplain","sTillNotSure","juStWaitHere",
    "mYlastTry","nOrealAnswer","iDontMind","thoSomehowFine","wEllMaybe",
    "silentorbit","velvetpulse","neonwhisper","lunarhabit","frostecho",
    "urbanmirage","hazydrift","nightartifact","solsticeframe","brokenzen",
    "staticbloom","wildparadox","cobaltveil","dreampayload","ghostsyntax",
    "ambervoid","pixelwander","ironhorizon","fadedcompass","lucidtempo",
    "softrebellion","vaporlogic","hiddenvector","astralhabit","quietoverdrive",
    "randompilgrim","crystalglitch","humanarchive","digitalnomicon","blurredgravity",
    "neonmonk","voidjournal","retroshade","sonicdrifter","futurehabit",
    "echooperator","slowartifact","urbanzenith","palehacker","brokenradar",
    "zeropersona","shadowinput","nightconsumer","wiredpoetry","fadednetwork",
    "ghosttraveler","astralengine","humanfragment","staticoperator","distantkernel",
    "velvetanomaly","softglitch","pixelnomad","chromeheart","beatlost",
    "syntaxlunar","hackerneon","philosophydream","executorrandom","cortexsilent",
    "dramacode","wanderervoid","mechanicurban","illusion",
  ];
  for (let i = 0; i < 30; i++) {
...
function pvpAutoStartFromAcceptState(state) {
  const s = asObj(state);
  if (String(s.phase || "") !== "accept_match") return s;
  const am = asObj(s.acceptMatch);
  const deadlineMs = Number(am.deadlineMs || 0);
  const now = Date.now();
  if (!(deadlineMs > 0) || now < deadlineMs) return s;
  return {
    ...s,
    phase: String(am.nextPhase || "turn_input"),
    phaseAtMs: now,
    acceptMatch: { ...am, done: true, autoStartedAtMs: now },
    updatedAt: new Date().toISOString(),
  };
}
...
async function pvpGetRoomState(initData, roomId) {
  ...
  if (String(nextRoom?.status || "") === "active" && String(nextState.phase || "") === "accept_match") {
    const forced = pvpAutoStartFromAcceptState(nextState);
    if (String(forced.phase || "") !== "accept_match") {
