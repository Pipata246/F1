...
begin
  ...
  -- rest of function body unchanged, just stake validation updated
  ...
  if coalesce(v_p1_balance, 0) < p_stake_ton then raise exception 'Insufficient balance for selected stakes'; end if;
  ...
  if coalesce(v_p2_balance, 0) < p_stake_ton then raise exception 'Insufficient balance for selected stakes'; end if;

  update public.users set balance = round((coalesce(balance,0) - p_stake_ton)::numeric,9), updated_at = now()
    where tg_user_id = v_room.player1_tg_user_id;
  update public.users set balance = round((coalesce(balance,0) - p_stake_ton)::numeric,9), updated_at = now()
    where tg_user_id = p_player2_tg_user_id;

  update public.pvp_rooms
    set player2_tg_user_id = left(coalesce(p_player2_tg_user_id,''),64),
